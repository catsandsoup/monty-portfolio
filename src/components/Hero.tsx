...
const Hero = () => {
  ...
  const achievementCards = achievements.map((achievement, index) => <motion.div key={achievement.label} initial={{
    opacity: 0,
    y: 10
  }} animate={{
    opacity: 1,
  ...
  return <section className="relative min-h-screen w-full overflow-hidden" ref={containerRef}>
      ...
      <div className="relative z-10 w-full px-4 sm:px-6 mx-auto py-8 mt-12">
        {isLoaded && <motion.div variants={optimizedFadeIn} {...optimizedAnimationProps} className="max-w-4xl mx-auto text-center pt-12 md:pt-24" style={{
        willChange: 'opacity, transform'
      }}>
          ...
          willChange: 'opacity'
              ...
              <div className="w-40 h-40 mx-auto rounded-full overflow-hidden border-4 border-[#00a5ee]/20 transition-all duration-300 hover:scale-105">
                <img src="/monty-profile.png" alt="Monty Giovenco" className="w-full h-full object-cover" loading="eager" width={160} height={160} />
              </div>
            </motion.div>

            <motion.h1 variants={optimizedFadeIn} {...optimizedAnimationProps} className="text-4xl sm:text-5xl md:text-6xl font-bold mb-6 bg-clip-text text-transparent bg-gradient-to-r from-gray-900 to-gray-600 px-4 py-4" style={{
          willChange: 'opacity, transform'
        }}>
              Monty Giovenco
            </motion.h1>

            <motion.div variants={optimizedFadeIn} {...optimizedAnimationProps} className="relative mb-12 px-4" style={{
          willChange: 'opacity, transform'
        }}>
            ...
            <motion.div variants={optimizedFadeIn} {...optimizedAnimationProps} transition={{
          delay: 0.4
        }} className="flex flex-col sm:flex-row justify-center items-center gap-4 sm:gap-8 px-4" style={{
          willChange: 'opacity, transform'
        }}>
